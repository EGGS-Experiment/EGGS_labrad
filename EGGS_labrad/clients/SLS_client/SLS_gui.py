...
class SLS_gui(QFrame):
    ...
    def _makeOffsetWidget(self):
        ...
        offset_lockpoint_label.setFont(LABEL_FONT)
        offset_eom_rf_amp_label = QLabel("EOM RF Ampl.")
        offset_eom_rf_amp_label.setFont(LABEL_FONT)
        offset_freq_label = QLabel("Offset Freq.")
        offset_freq_label.setFont(LABEL_FONT)

        offset_buttons = {
            'offset_freq': (10.0, 800.0, 1.0, ' MHz'),
            'offset_eom_rf_amp': (0, 100.0, 0.1, ' %')
        }
        for offset_button_name, button_vals in offset_buttons.items():
            val_min, val_max, val_step, val_suffix = button_vals
            setattr(self, offset_button_name, QCustomUnscrollableSpinBox())
            offset_button = getattr(self, offset_button_name)
            offset_button.setRange(val_min, val_max)
            offset_button.setSingleStep(val_step)
            offset_button.setSuffix(val_suffix)
            offset_button.setAlignment(Qt.AlignRight)
            offset_button.setFont(INPUT_FONT)

        self.offset_lockpoint = QCustomUnscrollableComboBox()
        self.offset_lockpoint.addItems(["J(+2)", "J(+1)", "Resonance", "J(-1)", "J(-2)"])
        self.offset_lockpoint.setFont(INPUT_FONT)
    ...
    def _makePDHWidget(self):
        ...
        pdh_filter_label.setFont(LABEL_FONT)
        pdh_phasemodulation_label = QLabel("Phase Mod.")
        pdh_phasemodulation_label.setFont(LABEL_FONT)
        pdh_phaseoffset_label = QLabel("Ref. Phase")
        pdh_phaseoffset_label.setFont(LABEL_FONT)
        pdh_freq_label = QLabel("PDH Freq.")
        pdh_freq_label.setFont(LABEL_FONT)

        pdh_buttons = {
            'pdh_freq': (10.0, 35.0, 0.1, " MHz"),
            'pdh_phaseoffset': (0, 360.0, 0.1, " rad"),
            'pdh_phasemodulation': (0, 3.0, 0.1, " deg")
        }
        for pdh_button_name, button_vals in pdh_buttons.items():
            ...
            pdh_button.setRange(val_min, val_max)
            pdh_button.setSingleStep(val_step)
            pdh_button.setSuffix(val_suffix)
            pdh_button.setAlignment(Qt.AlignRight)
            pdh_button.setFont(INPUT_FONT)

        self.pdh_filter = QCustomUnscrollableComboBox()
        self.pdh_filter.addItems(["None"] + [str(val) for val in range(1,16)])
        self.pdh_filter.setFont(INPUT_FONT)
    ...
    def _makeServoWidget(self):
        ...
        servo_param_label.setFont(LABEL_FONT)
        servo_set_label = QLabel("Setpoint")
        servo_set_label.setFont(LABEL_FONT)
        servo_p_label = QLabel("P (Proportional)")
        servo_p_label.setFont(LABEL_FONT)
        servo_i_label = QLabel("I (Integral)")
        servo_i_label.setFont(LABEL_FONT)
        servo_d_label = QLabel("D (Differential)")
        servo_d_label.setFont(LABEL_FONT)
        servo_filter_label = QLabel("Filter Index")
        servo_filter_label.setFont(LABEL_FONT)

        self.servo_filter = QCustomUnscrollableComboBox()
        self.servo_filter.addItems(["None"] + [str(val) for val in range(1,16)])
        self.servo_filter.setFont(INPUT_FONT)
        ...
        for servo_button_name, button_vals in servo_buttons.items():
            ...
            servo_button.setAlignment(Qt.AlignRight)
            servo_button.setFont(INPUT_FONT)

        self.servo_param = QCustomUnscrollableComboBox()
        self.servo_param.addItems(["Current", "PZT", "TX"])
        self.servo_param.setFont(INPUT_FONT)
        ...
        return QCustomGroupBox(servo_widget, "Servo")

    def makeLayout(self):
        # make constituent widgets
        ...
        sls_label.setFont(QFont(SHELL_FONT, pointSize=18))
        ...
        for i in range(4):
            layout.setColumnMinimumWidth(i, 185)
