...
class SidebarTabImage(QWidget):
    ...
    def _makeLayout(self):
        ...
        self.rotation.addItems(['todo: idk'])

        # flip image
        flip_label = QLabel("Flip Image:")
        flip_label.setAlignment(_ANDOR_ALIGNMENT)
        self.flip = QComboBox()
        self.flip.setFont(QFont(SHELL_FONT, pointSize=12))
        self.flip.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.flip.addItems(['todo: idk'])

        # create section widget
        display_holder = QWidget()
        display_widget_layout = QGridLayout(display_holder)
        # lay out section
        display_widget_layout.addWidget(flip_label,         0, 0, 1, 1)
        display_widget_layout.addWidget(self.flip,          0, 1, 1, 1)
