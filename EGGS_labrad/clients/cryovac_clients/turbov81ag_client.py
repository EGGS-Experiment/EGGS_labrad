...
class turbov81ag_client(GUIClient):

    name = 'Turbo-V81 AG Client'
    ...
    def initClient(self):
        ...
        yield self.tv.signal__pressure_update(self.PRESSUREID)
        yield self.tv.addListener(listener=self.updatePressure, source=None, ID=self.PRESSUREID)
        yield self.tv.signal__power_update(self.POWERID)
        yield self.tv.addListener(listener=self.updatePower, source=None, ID=self.POWERID)
        yield self.tv.signal__speed_update(self.SPEEDID)
        yield self.tv.addListener(listener=self.updateSpeed, source=None, ID=self.SPEEDID)
        yield self.tv.signal__toggle_update(self.TOGGLEID)
        yield self.tv.addListener(listener=self.updateToggle, source=None, ID=self.TOGGLEID)
        # set recording stuff
        self.c_record = self.cxn.context()
        self.recording = False
        self.starttime = None
        # start device polling only if not already started
        poll_params = yield self.tv.polling()
        if not poll_params[0]:
            yield self.tv.polling(True, 5.0)

    @inlineCallbacks
    def initData(self):
        status_tmp = yield self.tv.toggle()
    ...
    def initGUI(self):
        ...
        self.gui.twistorr_toggle.clicked.connect(lambda status: self.tv.toggle(status))
    ...
    def record_pressure(self, status):
        ...
        if status:
            ...
            yield self.dv.new('Turbo-V 81AG Pump Controller', [('Elapsed time', 't')],
                              [('Pump Pressure', 'Pressure', 'mbar')], context=self.c_record)


if __name__ == "__main__":
    from EGGS_labrad.clients import runClient
    runClient(turbov81ag_client)
