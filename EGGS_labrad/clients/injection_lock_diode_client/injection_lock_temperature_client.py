...
class InjectionLockTemperatureClient(GUIClient):
    ...
    def initClient(self):
        ...
        yield self.tec.signal__toggle_update(TOGGLEID)
        yield self.tec.addListener(listener=self.updateToggle, source=None, ID=TOGGLEID)
        yield self.tec.signal__current_update(CURRENTID)
        yield self.tec.addListener(listener=self.updateCurrent, source=None, ID=CURRENTID)
        yield self.tec.signal__temperature_update(TEMPERATUREID)
        yield self.tec.addListener(listener=self.updateTemperature, source=None, ID=TEMPERATUREID)
        yield self.tec.signal__lock_update(LOCKID)
        yield self.tec.addListener(listener=self.updateLock, source=None, ID=LOCKID)
        yield self.tec.signal__setpoint_update(SETPOINTID)
        yield self.tec.addListener(listener=self.updateSetpoint, source=None, ID=SETPOINTID)
    ...
    def initGUI(self):
        ...
        self.gui.lock_set.lineEdit().returnPressed.connect(
            lambda _box=self.gui.lock_set, _device_func=self.tec.locking_setpoint:
            self.value_changed(None, _box, _device_func))

        self.gui.lock_P.textChanged.connect(lambda _: self.gui.lock_P.blockSignals(True))
        self.gui.lock_P.lineEdit().returnPressed.connect(
            lambda _box=self.gui.lock_P, _device_func=self.tec.locking_p:
            self.value_changed(None, _box, _device_func))

        self.gui.lock_I.textChanged.connect(lambda _: self.gui.lock_I.blockSignals(True))
        self.gui.lock_I.lineEdit().returnPressed.connect(
            lambda _box=self.gui.lock_I, _device_func=self.tec.locking_i:
            self.value_changed(None, _box, _device_func))

        self.gui.lock_D.textChanged.connect(lambda _: self.gui.lock_D.blockSignals(True))
        self.gui.lock_D.lineEdit().returnPressed.connect(
            lambda _box=self.gui.lock_D, _device_func=self.tec.locking_d:
            self.value_changed(None, _box, _device_func))
    ...
    def updateToggle(self, c, status):
        ...
        if (not toggleswitch.signalsBlocked()) and (not toggleswitch.hasFocus()):
    ...
    def updateSetpoint(self, c, setpoint):
        ...
        """
        if (not self.gui.lock_set.signalsBlocked()) and (not self.gui.lock_set.hasFocus()):
    ...
    def updateLock(self, c, msg):
        ...
        # set value
        if (not widget.signalsBlocked()) and (not widget.hasFocus()):
